...
[1.什么是增量目标检测？它与普通目标检测的核心区别是什么？](#1.什么是增量目标检测？它与普通目标检测的核心区别是什么？)

[2.在增量目标检测中，"背景类语义漂移（Background Shift）"是什么问题？如何解决？](#2.在增量目标检测中，"背景类语义漂移（Background Shift）"是什么问题？如何解决？)

[3.增量目标检测中基于知识蒸馏的方法是如何工作的？请介绍代表性工作。](#3.增量目标检测中基于知识蒸馏的方法是如何工作的？请介绍代表性工作。)

[4.增量目标检测的主流评估协议有哪些？如何设计实验来衡量一个模型的遗忘程度？](#4.增量目标检测的主流评估协议有哪些？如何设计实验来衡量一个模型的遗忘程度？)

[5.如何将 DETR 架构应用于增量目标检测？它相比基于 Faster R-CNN 的方法有什么天然优势？](#5.如何将 DETR 架构应用于增量目标检测？它相比基于 Faster R-CNN 的方法有什么天然优势？)

[6.增量目标检测与开放世界检测（OWD）、开放词汇检测（OVD）有什么本质区别？](#6.增量目标检测与开放世界检测（OWD）、开放词汇检测（OVD）有什么本质区别？)
...
<h2 id="1.什么是增量目标检测？它与普通目标检测的核心区别是什么？">1.什么是增量目标检测？它与普通目标检测的核心区别是什么？</h2>

增量目标检测（Incremental Object Detection，也称持续学习目标检测）指模型在**不访问旧任务训练数据**的前提下，持续学习新类别的检测能力，同时保持对已学类别的检测性能。

与普通目标检测的核心区别体现在三个方面：

**1. 数据访问限制**：普通检测器在训练时可以访问所有类别的数据，而增量检测器在学习新类别时，旧类别的训练数据通常已不可用（出于隐私、存储或成本的限制）。

**2. 类别集合动态扩展**：普通检测器的类别集合在训练前固定，增量检测器需要支持类别集合的持续扩展，例如从检测"猫、狗"扩展到"猫、狗、汽车、行人"。

**3. 灾难性遗忘的威胁**：普通检测器不存在遗忘问题，而增量检测器面临的核心挑战是——在优化新类别损失时，梯度更新会覆盖旧类别的特征表示，导致旧类别性能急剧下降，即**灾难性遗忘（Catastrophic Forgetting）**。

**实际应用场景举例**：

- 自动驾驶系统上线后，需要持续学习识别新型车辆或新型交通标志
- 工业质检系统需要在不停线的情况下增加新缺陷类别的检测能力
- 安防系统随业务扩展需要持续增加可识别的目标类型

<h2 id="2.在增量目标检测中，"背景类语义漂移（Background Shift）"是什么问题？如何解决？">2.在增量目标检测中，"背景类语义漂移（Background Shift）"是什么问题？如何解决？</h2>

### 问题定义

背景类语义漂移是增量目标检测中特有的、比分类增量学习更复杂的挑战。

以一个具体例子说明：

- **第一阶段**：模型学习检测"猫"和"狗"，此时图像中出现的"汽车"被当作**背景负样本**处理，模型学会了将"汽车区域"压制为背景。
- **第二阶段**：引入"汽车"类别。如果此时使用第一阶段的图像做回放训练，这些图像中的"汽车"依然被标注为背景，与新阶段"汽车是前景"的监督信号产生**语义矛盾**。

这种矛盾会导致模型对新类别的学习产生严重干扰，是检测增量学习远比分类增量学习困难的根本原因之一。

### 主要解决方案

**方案一：忽略标签（Ignore Label）**

对旧阶段图像中可能属于新类别的区域，打上"忽略"标记，使其既不参与正样本损失，也不参与负样本损失的计算，避免监督信号冲突。

```python
# 伪代码示例
for region in old_image_regions:
    if iou(region, potential_new_class_region) > threshold:
        region.label = IGNORE  # 不计入损失
    else:
        region.label = BACKGROUND
```

**方案二：伪标签（Pseudo Label）**

用当前阶段已具备新类别知识的模型，对旧阶段图像进行推理，将高置信度的新类别预测框作为正样本标注加入训练，动态修正旧图像的标签。

**方案三：不使用旧数据回放**

彻底避免回放旧图像，转而使用知识蒸馏等方式在特征层面约束模型，从根本上绕开背景漂移问题。

<h2 id="3.增量目标检测中基于知识蒸馏的方法是如何工作的？请介绍代表性工作。">3.增量目标检测中基于知识蒸馏的方法是如何工作的？请介绍代表性工作。</h2>

### 核心思想

用第 $t-1$ 阶段训练完毕的旧模型（Teacher）来监督第 $t$ 阶段正在训练的新模型（Student），通过蒸馏损失约束新模型的特征表示不发生剧烈漂移，从而保留旧类别的检测能力。

总损失函数通常形式如下：

$$\mathcal{L}_{total} = \mathcal{L}_{det}^{new} + \lambda \cdot \mathcal{L}_{distill}$$

其中 $\mathcal{L}_{det}^{new}$ 是新类别的检测损失，$\mathcal{L}_{distill}$ 是蒸馏损失，$\lambda$ 控制两者的平衡。

### 代表性工作

**ILOD（2017）**：

- 首个将知识蒸馏引入检测增量学习的工作，基于 Faster R-CNN 框架。
- 对 RPN 的输出置信度和 RoI 特征向量同时施加蒸馏损失。
- 蒸馏损失形式：$\mathcal{L}_{distill} = \|f_{new}(x) - f_{old}(x)\|_2^2$
- 局限性：只约束了检测头，未对 Backbone 特征施加约束，特征漂移依然存在。

**Faster ILOD**：

- 在 ILOD 基础上引入**特征图级别的蒸馏**，对中间层特征图直接施加 MSE 损失。
- 相比 ILOD，更有效地抑制了 Backbone 的特征漂移。

**MMA（Maintaining Discrimination and Fairness）**：

- 指出直接蒸馏存在旧类别主导的问题：旧模型对旧类别的输出置信度远高于新类别，蒸馏会让模型偏向旧类别。
- 提出对新旧类别分别施加公平性约束，平衡新旧类别的学习权重。

### 蒸馏方法的局限

蒸馏强度 $\lambda$ 的选择非常敏感：$\lambda$ 过大会限制新类别的学习（可塑性不足），$\lambda$ 过小则无法有效防止遗忘（稳定性不足），这一矛盾被称为 **Stability-Plasticity Dilemma**（稳定性-可塑性困境）。

---

<h2 id="4.增量目标检测的主流评估协议有哪些？如何设计实验来衡量一个模型的遗忘程度？">4.增量目标检测的主流评估协议有哪些？如何设计实验来衡量一个模型的遗忘程度？</h2>

### 主流评估协议

**协议一：VOC → COCO 迁移协议**

先在 PASCAL VOC 的 20 个类别上完整训练，再增量学习 COCO 数据集中额外引入的新类别，评估最终在全部类别上的 mAP。这种协议适合评估跨数据集增量学习的泛化能力。

**协议二：分阶段 COCO 协议**（最常用）

将 COCO 80 个类别按不同划分方式分批引入：

| 划分方式    | 描述                  | 难度 |
| ----------- | --------------------- | ---- |
| 40 + 40     | 2个阶段，每阶段40类   | 较低 |
| 70 + 10     | 2个阶段，最后引入10类 | 中等 |
| 10 × 8      | 8个阶段，每阶段10类   | 较高 |
| 40 + 1 × 40 | 逐类引入后40类        | 最高 |

阶段数越多、每次引入类别越少，模型面临的增量压力越大，遗忘问题越严重。

### 核心评估指标

- $\text{Old AP}$：模型在旧类别上的 mAP，直接衡量**遗忘程度**，越高越好。
- $\text{New AP}$：模型在新类别上的 mAP，衡量**新知识学习效果**，越高越好。
- $\text{All AP}$：全部类别的综合 mAP，是最终性能的核心指标。

**遗忘率（Forgetting Rate）**的定量计算：

$$\text{FR} = \frac{\text{AP}_{old}^{joint} - \text{AP}_{old}^{incremental}}{\text{AP}_{old}^{joint}} \times 100\%$$

其中 $\text{AP}_{old}^{joint}$ 是用全部数据联合训练（Joint Training）时旧类的 AP（视为上界），$\text{AP}_{old}^{incremental}$ 是增量训练后旧类的 AP。遗忘率越低，说明模型的持续学习能力越强。

### 两个重要基线

- **Joint Training（联合训练）**：用所有阶段的全部数据一起训练，作为**性能上界**。
- **Fine-tuning（直接微调）**：不加任何防遗忘机制，直接在新数据上训练，作为**遗忘程度下界**（灾难性遗忘的最坏情况）。

<h2 id="5.如何将 DETR 架构应用于增量目标检测？它相比基于 Faster R-CNN 的方法有什么天然优势？">5.如何将 DETR 架构应用于增量目标检测？它相比基于 Faster R-CNN 的方法有什么天然优势？</h2>

### DETR 用于增量检测的核心思路

DETR 使用固定数量的**对象查询（Object Queries）**来预测目标，这种基于查询的预测机制天然适合增量扩展：

**方法一：查询扩展（Query Expansion）**

为每个新阶段引入的新类别分配专属的新查询向量，同时**冻结**旧阶段查询的参数，使旧类别的预测能力得以保留。

```
阶段1：[Q1, Q2, ..., Q100]  → 检测旧类别
阶段2：[Q1, Q2, ..., Q100（冻结）, Q101, ..., Q120（新增）] → 检测全部类别
```

**方法二：查询蒸馏（Query Distillation）**

对旧阶段查询的输出（预测类别和边界框）施加蒸馏损失，约束新模型的旧类别预测与旧模型保持一致，在不冻结参数的情况下减少遗忘。

### 相比 Faster R-CNN 的天然优势

| 维度       | Faster R-CNN 系方法                    | DETR 系方法                      |
| ---------- | -------------------------------------- | -------------------------------- |
| 候选框生成 | RPN 对未知类别泛化能力有限             | 注意力机制对新类别有更好的泛化性 |
| 类别扩展   | 分类头需要整体扩展，旧类别权重易被干扰 | 对象查询可独立扩展，天然解耦     |
| 特征交互   | 局部卷积感受野受限                     | 全局自注意力建模，特征更具判别力 |
| 端到端性   | RPN + RoI Head 两阶段，蒸馏位置较多    | 端到端结构，蒸馏目标更统一       |

### 当前挑战

DETR 在增量场景下也存在**匹配不稳定**的问题——匈牙利匹配的结果随训练数据变化而波动，导致同一真实目标在不同训练迭代中可能被分配给不同的查询向量，增加了知识蒸馏的难度。目前有工作通过**一致性匹配损失（Consistent Matching Loss）**来缓解这一问题。

<h2 id="6.增量目标检测与开放世界检测（OWD）、开放词汇检测（OVD）有什么本质区别？">6.增量目标检测与开放世界检测（OWD）、开放词汇检测（OVD）有什么本质区别？</h2>

### 三者的本质区别

| 维度                 | 增量目标检测（IOD）                                    | 开放世界检测（OWD）                                  | 开放词汇检测（OVD）                        |
| -------------------- | ------------------------------------------------------ | ---------------------------------------------------- | ------------------------------------------ |
| **核心问题**         | 如何在不访问旧数据的情况下学习新类别，同时不遗忘旧类别 | 如何发现并标记图中未知类别的物体，再通过增量学习吸收 | 如何在推理时检测训练集中从未出现过的新类别 |
| **对新类别的处理**   | 需要提供新类别的标注数据进行增量训练                   | 先标记为"Unknown"，待人工标注后再学习                | 仅需提供新类别的文本描述，无需任何训练     |
| **是否需要重新训练** | 需要（增量训练）                                       | 需要（增量吸收）                                     | 不需要（Zero-shot 推理）                   |
| **未知类别发现**     | 不具备                                                 | 核心能力                                             | 不具备（依赖用户提供类别名）               |
| **依赖条件**         | 新类别的图像标注数据                                   | 无需提前知道新类别                                   | 预训练的视觉语言模型（如CLIP）             |
| **代表方法**         | ILOD, Faster ILOD, MMA                                 | ORE, OW-DETR                                         | GLIP, Grounding DINO, YOLO-World           |

### 三者关系

```
开放世界检测（OWD）
    ├── 未知目标发现（Unknown Detection）← 独有能力
    └── 增量学习吸收新类别 ← 与 IOD 有重叠
